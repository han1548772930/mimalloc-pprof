...
`test-arena-reclaim-reset` repeats them with `MIMALLOC_PURGE_DECOMMITS=0`: a reset
keeps data slices committed, so reclaim may debit those slices as well as metadata.
