...
static void mi_arena_schedule_purge(mi_arena_t* arena, size_t slice_index, size_t slice_count) {
  ...
  if (delay == 0) {
    ...
    if (mi_atomic_casi64_strong_acq_rel(&arena->purge_expire, &expire0, expire)) {
      ...
      // opportunistic path checks (this function's caller, `_mi_arenas_try_purge`, returns
      // early while it is 0) and the only deadline the scavenger waits on, and it is
      // cleared to 0 both by that function's settle CAS and by the scavenger's own
      // stale-value CAS, while this arena's expire can stay set. Once the two disagree,
      // every opportunistic purge is skipped and the scavenger parks on its safety net, so
      // free arena space is only returned when something forces `mi_collect(true)`.
      //
      // Re-arm from this arena's own pending deadline, which is what the settle CAS would
      // have written, so this restores the intended invariant rather than adding a policy.
      // The CAS keeps an earlier value if one is already there, and the relaxed pre-check
      // keeps the common case down to a plain load.
